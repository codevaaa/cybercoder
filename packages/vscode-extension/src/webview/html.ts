...
case 'status':if(assistantEl){let trace=assistantEl.querySelector('.agent-trace');if(!trace){trace=document.createElement('div');trace.className='tool-step agent-trace';trace.style.borderColor='rgba(139,92,246,0.3)';trace.style.marginTop='8px';trace.innerHTML='<div class="tool-header" style="background:rgba(139,92,246,0.05)"><span class="tool-icon" style="color:var(--purple)">✦</span><span class="tool-name">Agent Trace</span><span class="tool-chevron">▸</span></div><div class="tool-body trace-body" style="color:#b48bed"></div>';trace.querySelector('.tool-header').onclick=()=>trace.classList.toggle('expanded');assistantEl.appendChild(trace)}const ln=document.createElement('div');ln.textContent='> '+m.text;trace.querySelector('.trace-body').appendChild(ln);assistantEl.scrollIntoView({block:'end',behavior:'smooth'})}break;
